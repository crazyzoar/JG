...
namespace JG_Prospect
{
    ...
    public partial class EditUser : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            DataSet dsadd = InstallUserBLL.Instance.GetRejectionReason();
            if (dsadd.Tables[0].Rows.Count > 0)
            {
                ddlReject.DataSource = dsadd.Tables[0];
                ddlReject.DataTextField = "Reason";
                ddlReject.DataValueField = "Reason";
                ddlReject.DataBind();
                ddlReject.Items.Insert(0, "Select Rejection Reason");
                ddlReject.SelectedIndex = 0;
                //txtReject.Text = "";
            }
            else
            {
                ddlReject.DataSource = dsadd;
                ddlReject.DataBind();
                ddlReject.Items.Add("Select Rejection Reason");
                ddlReject.SelectedIndex = 0;
            }
        }
        ...
        protected void btnSaveReason_Click(object sender, EventArgs e)
        {
            if (Convert.ToString(Session["DeactivationStatus"]) == "Deactive")
            {
                ...
                string EmpType = "";
                string PayRates = "";
                ds = InstallUserBLL.Instance.ChangeStatus(Convert.ToString(Session["EditStatus"]), 
                    Convert.ToInt32(Session["EditId"]), DateTime.Today, DateTime.Now.ToShortTimeString(), 
        ...
        #region Code added by Sonali Agarwal on 21Apr,2017
        protected void btnAddReject_Click(object sender, EventArgs e)
        {
            if (txtReject.Text != "")
            {
                string reason = txtReject.Text;
                DataSet ds = InstallUserBLL.Instance.CheckRejectionReason(reason);
                if (ds.Tables[0].Rows.Count > 0)
                {
                    //ScriptManager.RegisterStartupScript(this, this.GetType(), "alert", "alert('Reject Reason already exists.')", true);
                }
                else
                {
                    DataSet dsadd = InstallUserBLL.Instance.AddRejectionReason(reason);
                    if (dsadd.Tables[0].Rows.Count > 0)
                    {
                        ddlReject.DataSource = dsadd.Tables[0];
                        ddlReject.DataTextField = "Reason";
                        ...
                        ddlReject.DataBind();
                        ddlReject.Items.Insert(0, "Select Rejection Reason");
                        ddlReject.SelectedValue = reason;
                        txtReject.Text = "";
                    }
                }
            ...
            //ScriptManager.RegisterStartupScript(this, this.GetType(), "Overlay", "overlayIssue()", true);
        }
        protected void btnDeleteReject_Click(object sender, EventArgs e)
        {
            if (txtReject.Text != "")
            {
                ...
                if (ds.Tables[0].Rows.Count == 0)
                {
                    //ScriptManager.RegisterStartupScript(this, this.GetType(), "alert", "alert('Reject Reason does not exists.')", true);
                }
                else
                {
                    InstallUserBLL.Instance.DeleteRejectionReason(reason);
                    DataSet dsadd = InstallUserBLL.Instance.GetRejectionReason();
                    if (dsadd.Tables[0].Rows.Count > 0)
                    {
                        ddlReject.DataSource = dsadd.Tables[0];
            ...
            //ScriptManager.RegisterStartupScript(this, this.GetType(), "Overlay", "overlay()", true);return;
        }
